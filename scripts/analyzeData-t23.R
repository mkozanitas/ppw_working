...
nrow(allIndv)
plot.ok <- allIndv$Num[which(allIndv$P18==allIndv$P19)] 
spec.ok <- allIndv$Num[which(allIndv$S18==allIndv$S19)] 
...
# rows are 2018 state, cols are 2019 state
# numbers along diagonal are trees that were in same state in 18 and 19
#  row 1, after col 1, are individuals recorded as dead in 18 that are NOT dead in 19
...
######## BELOW HERE ALL FROM analyzeData.R original file
