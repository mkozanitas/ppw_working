...
#####from here change all x to 13 all y to 18 and all t12 to t1234
...
# SEARCH AND REPLACXE FROM HERE
t12$Quad.13[newIndvs] <- t12$Quad.18[newIndvs]
t12$Type.13[newIndvs] <- t12$Type.18[newIndvs]
t12$Species.13[newIndvs] <- t12$Species.18[newIndvs]

# This introduces error because 2018 basal areas reflect 5 more years of growth
t12$Basal.Area.13[newIndvs] <- t12$Basal.Area.18[newIndvs]
t12$dbh.13[newIndvs] <- t12$dbh.18[newIndvs]
t12$Dead.13[newIndvs] <- 0
t12$Live.13[newIndvs] <- 1
t12$gCrown.13[newIndvs] <- 1

# ignore these individuals for basal area growth
t12$UseForBAGrowth <- T
t12$UseForBAGrowth[newIndvs] <- F

#plot(t12$Basal.Area.13,t12$Basal.Area.18)
#summary(t12$Basal.Area.18/t12$Basal.Area.13)
#abline(0,1)
...
for (i in 1:nrow(fst))
{
  sp <- fst$Species[i]
  ty <- fst$Type[i]
  init <- t12$Num[which(t12$Species.13==sp & t12$Type.13==ty & t12$Live.13==1)]
  fst$N13[i] <- length(init)
  
  fin1 <- intersect(init,t12$Num[which(t12$Live.18==0)])
  ...
  fin2 <- intersect(init,t12$Num[which(t12$bSprout.18==1 & t12$Topkill.18==1)])
...
t12$fsLevel[which(t12$Plot.13 %in% unburned.plots)] <- 0
table(t12$fsLevel)

table(t12$Plot.13[which(t12$fsLevel==3)])
...
hist(t12$dbh.13)
t12$ldbh <- log10(t12$dbh.13)
hist(t12$ldbh)
t12$ldbh2 <- t12$ldbh^2

#### SURVIVAL ANALYSIS
plot(t12$ldbh,t12$Live.18)

# **** mixing SA and TR - need to work on diameter conversion to get this right ***
(N <- length(which(!is.na(t12$ldbh) & !is.na(t12$Live.18))))
fit <- glm(Live.18~ldbh,data=t12,family='binomial')
summary(fit)
...
{
  (species <- spA[i])
  tmp <- t12[which(t12$Species.13==species),]
...
t12s <- t12[which(t12$Species.13 %in% spA & t12$fsLevel %in% c(2:3)),]
...
table(t12s$Species.13)
head(t12s)
fit <- glm(rVar~ldbh + ldbh2 + Species.13,data=t12s,family='binomial')
summary(fit)

summary(t12s$ldbh)
#choose sizes here at which predicted values will be calculated
predSizes <- c(10,30,80)
nd <- with(t12s,data.frame(ldbh=rep(log10(predSizes),length(spA)),Species.13=rep(spA,each=length(predSizes))))
nd$ldbh2 <- nd$ldbh^2
nd
...
t12s <- t12[which(t12$Species.13 %in% spA & t12$fsLevel %in% c(0:3)),]
...
# plot(t12s$FireSev,t12s$Live.18)
# i=1
# for (i in 1:nvals) {
...
{
  fit2 <- glm(Live.18~ldbh + as.factor(fsLevel) + Species.13,data=t12s,family='binomial')
  fit1 <- glm(Live.18~ldbh + as.factor(fsLevel) + Species.13+ as.factor(fsLevel):Species.13,data=t12s,family='binomial')
  BIC(fit1)
  BIC(fit2)
  summary(fit1)
  summary(fit2)
  ...
  plotSpecies <- function(spname,nd.tmp=nd2) {
    tmp <- t12s[which(t12s$Species.13==spname),]
    #op=par(mfrow=c(1,2))
    
    # plot(tmp$FireSev,tmp$Live.18,main=spname,xlim=range(t12s$FireSev,na.rm=T))
    # i=1
    # for (i in 1:nvals) {
    ...
    plot(tmp$ldbh,tmp$Live.18,xlim=range(t12s$ldbh,na.rm=T))
    i=2
...
{
  fit2 <- glm(gCrown.18~ldbh + as.factor(fsLevel) + Species.13,data=t12s,family='binomial')
  fit1 <- glm(gCrown.18~ldbh + as.factor(fsLevel) + Species.13+ as.factor(fsLevel):Species.13,data=t12s,family='binomial')
  BIC(fit1)
  BIC(fit2)
  summary(fit1)
  summary(fit2)
  ...
  plotSpecies <- function(spname,nd.tmp=nd2) {
    ...
    plot(tmp$ldbh,tmp$gCrown.18,xlim=range(t12s$ldbh,na.rm=T))
    i=2
    for (i in 1:length(unique(nd.tmp$fsLevel))) {
      ndt <- nd.tmp[which(nd2$Species.13==spname & nd.tmp$fsLevel==fsLevels.vals[i]),]
      lines(ndt$ldbh,ndt$pSurvAll)
...
table(t12$TB)
table(t12$gCrown.18)
table(t12$Live.18,t12$TB)
table(t12$Live.18,t12$gCrown.18)

t12$PFstatus <- (-1)
t12$PFstatus[which(t12$Live.18==0)] <- 0
t12$PFstatus[which(t12$TB==1)] <- 1
t12$PFstatus[which(t12$gCrown.18==1)] <- 2
table(t12$PFstatus)
...
spArows <- which(t12$Species.13 %in% spA)
...
t12sp <- t12[which(t12$Species.13 %in% c(selSpecies) & t12$fsLevel %in% c(2:3)),] #individual species?
{
  #t12sp <- t12[which(t12$Species.13 %in% spA),] #abundant species?
  #t12sp <- t12[which(t12$Species.13 %in% spA & t12$fsLevel>1),] #abundant sp with fs level of 1 or more?
  ...
  #plot(t12sp$ldbh,t12sp$Dead.18,xlim=t12ldbh.range)
  fit1 <- glm(Dead.18~ldbh,data=t12sp,family='binomial')
  fit2 <- glm(Dead.18~ldbh+ldbh2,data=t12sp,family='binomial')
  BIC(fit1)
  BIC(fit2)
  ...
  #GREEN CROWN
  #plot(t12sp$ldbh,t12sp$gCrown.18,xlim=t12ldbh.range)
  fit1 <- glm(gCrown.18~ldbh,data=t12sp,family='binomial')
  fit2 <- glm(gCrown.18~ldbh+ldbh2,data=t12sp,family='binomial')
  summary(fit1)
...
#fit <- glmer(Live.18~ldbh + FireSev + Species.13 + (1 |Plot.13),data=t12s,family='binomial')

######## TOPKILL ANALYSIS
# current scoring has dead trees as topkilled. Change so topkill is only for those that are alive
t12$TopkillLive.18 <- 0
t12$TopkillLive.18[which(t12$Topkill.18==1 & t12$Live.18==1)] <- 1

table(t12$Dead.18,t12$TopkillLive.18,t12$gCrown.18)
t12[which(t12$gCrown.18==0 & t12$TopkillLive.18==0 & t12$Dead.18==0),]
...
op=par(mfrow=c(1,1))
plot(Dead.18~ldbh,data=t12a)
fitD <- glm(Dead.18~ldbh+ldbh2,data=t12a,family='binomial')
fitD
...
#plot(Topkill.18~ldbh,data=t12a)
fitT <- glm(TopkillLive.18~ldbh+ldbh2,data=t12a,family='binomial')
fitT
nd$pTopKill <- predict(fitT,nd,type='response')
lines(pTopKill~ldbh,data=nd,lwd=2,col='red')

#plot(gCrown.18~ldbh,data=t12a)
fitG2 <- glm(gCrown.18~ldbh+ldbh2,data=t12a,family='binomial')
fitG2
BIC(fitG2)
fitG1 <- glm(gCrown.18~ldbh,data=t12a,family='binomial')
fitG1
BIC(fitG1)
...
# Size only
rComp <- apply(t12[,c('ldbh','PFstatus','Species.13','FireSev')],1,allNotNA)
table(rComp)
t12a <- t12[rComp,]
...
head(t12)
dim(t12)
t12$dupStatusCheck <- apply(t12[,c('Dead.18','TopkillLive.18','gCrown.18')],1,sum,na.rm=T)
...
t12$PFstatus <- (-1)
t12$PFstatus[which(t12$Live.18==0)] <- 0
t12$PFstatus[which(t12$TopkillLive.18==1)] <- 1
t12$PFstatus[which(t12$Topkill.18==0 & t12$gCrown.18==1)] <- 2
table(t12$PFstatus)
...
# Add Species
fit3 <- multinom(PFstatus ~ ldbh + dFS + Species.13,data=t12a)
fit3
BIC(fit3)
...
fit3x <- multinom(PFstatus ~ ldbh + dFS + Species.13 + dFS:ldbh,data=t12a)
fit3x
BIC(fit3x)
...
rsel <- which(t12a$Species.13=='PSEMEN' & as.numeric(t12a$dFS)>0)
